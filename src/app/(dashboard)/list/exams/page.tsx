...
import { Class, Exam, Prisma, Subject, Teacher } from "@prisma/client"; // Importing Prisma types for type safety.
...
const ExamListPage = async ({
      ...
      <td>
        <div className="flex items-center gap-2"> {/* Actions column */}
          {(role === "admin" || role === "teacher") && ( // If the user is an admin or teacher, show the update and delete buttons.
            <>
              <FormContainer table="exam" type="update" data={item} /> {/* Update button */}
              <FormContainer table="exam" type="delete" id={item.id} /> {/* Delete button */}
            </>
          )}
        </div>
      </td>
  ...
  const query: Prisma.ExamWhereInput = {}; // Initializing a query object for filtering exams.
  ...
  switch (role) {
    case "admin":
      break; // Admin has access to all exams.
    case "teacher":
      query.lesson.teacherId = currentUserId!; // Filter exams by the teacher's ID.
      break;
    case "student":
      query.lesson.class = {
        students: {
          some: {
            id: currentUserId!, // Filter exams by the student's ID.
          },
        },
      };
      break;
    case "parent":
      query.lesson.class = {
        students: {
          some: {
            parentId: currentUserId!, // Filter exams by the parent's ID.
          },
        },
      };
      break;
  ...
  const [data, count] = await prisma.$transaction([ // Fetching exam data and total count using Prisma transactions.
    ...
    prisma.exam.count({ where: query }), // Count the total number of exams matching the query.
  ]);
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0"> {/* Main container */}
      ...
      <div className="flex items-center justify-between"> {/* Header section */}
        <h1 className="hidden md:block text-lg font-semibold">All Exams</h1> {/* Page title */}
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {(role === "admin" || role === "teacher") && (
              <FormContainer table="exam" type="create" /> /* Create exam button (admin or teacher only) */
            )}
          </div>
        </div>
      </div>
      {/* LIST */}
      <Table columns={columns} renderRow={renderRow} data={data} /> {/* Table displaying exam data */}
      {/* PAGINATION */}
...
export default ExamListPage; // Exporting the component as default.