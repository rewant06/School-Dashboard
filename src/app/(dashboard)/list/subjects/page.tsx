...
import { Prisma, Subject, Teacher } from "@prisma/client"; // Importing Prisma types for type safety.
...
type SubjectList = Subject & { teachers: Teacher[] }; // Defining a type that combines Subject and Teacher data.
...
const SubjectListPage = async ({
  ...
  const columns = [
    ...
    {
      header: "Teachers", // Column header.
      accessor: "teachers", // Key to access data for this column.
  ...
  const renderRow = (item: SubjectList) => (
    ...
    >
      <td className="flex items-center gap-4 p-4">{item.name}</td> {/* Subject name */}
      <td className="hidden md:table-cell">
        {item.teachers.map((teacher) => teacher.name).join(",")} {/* List of teacher names */}
      </td>
      <td>
        <div className="flex items-center gap-2"> {/* Actions column */}
          {role === "admin" && ( // If the user is an admin, show the update and delete buttons.
            <>
              <FormContainer table="subject" type="update" data={item} /> {/* Update button */}
              <FormContainer table="subject" type="delete" id={item.id} /> {/* Delete button */}
            </>
          )}
        </div>
      </td>
  ...
  const query: Prisma.SubjectWhereInput = {}; // Initializing a query object for filtering subjects.
  ...
  const [data, count] = await prisma.$transaction([ // Fetching subject data and total count using Prisma transactions.
    prisma.subject.findMany({
      where: query, // Applying filters.
      include: {
        teachers: true, // Include related teacher data.
      },
      take: ITEM_PER_PAGE, // Limit the number of items per page.
      skip: ITEM_PER_PAGE * (p - 1), // Skip items for pagination.
    }),
    prisma.subject.count({ where: query }), // Count the total number of subjects matching the query.
  ]);
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0"> {/* Main container */}
      ...
      <div className="flex items-center justify-between"> {/* Header section */}
        <h1 className="hidden md:block text-lg font-semibold">All Subjects</h1> {/* Page title */}
      ...
      {/* LIST */}
      <Table columns={columns} renderRow={renderRow} data={data} /> {/* Table displaying subject data */}
      {/* PAGINATION */}
...
export default SubjectListPage; // Exporting the component as default.